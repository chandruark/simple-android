...
import org.simple.clinic.phone.PhoneNumberMasker
...
class OverdueScreenControllerTest {
  ...
  private val numberMasker = mock<PhoneNumberMasker>()
  ...
  @Before
  fun setUp() {
    whenever(numberMasker.maskAndCall(any(), any())).thenReturn(Completable.complete())

    controller = OverdueScreenController(repository, numberMasker)
  ...
  fun `when call button is clicked, and permission result is received, call should be made using relevant method`(
    ...
    when (shouldUseDialer) {
      true -> verify(numberMasker).maskAndCall(number, Caller.UsingDialer)
      false -> verify(numberMasker).maskAndCall(number, Caller.WithoutDialer)
    }
    verifyNoMoreInteractions(screen)
  }
